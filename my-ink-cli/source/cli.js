...
const grid = Array.from({ length: 20 }, () => Array.from({ length: 20 }, () => (Math.random() > 0.5 ? 1 : 0)));
...
const boardState$ = interval(3000).pipe(startWith(generateInitialBoardState()), scan(board => calculateNextBoardState(board)));
...
const GameOfLife = () => {
    ...
    return (<Box flexDirection="column">
			{grid.map((row, i) => (<Box key={i}>
					{row.map((cell, j) => (<Text key={j}>{cell === 1 ? '■' : '□'}</Text>))}
				</Box>))}
		</Box>);
};
// Render the game
